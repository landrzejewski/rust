...
fn main() {
    ...
    */
    ...
    values.push(4); // błąd kompilacji, nie można modyfikować zmiennej, która dostępna z poziomu innej referencji
    println!("Value: {}", value);
}

#[allow(unused_variables)]
fn execute_task() {
    let number = Box::new(5); // zamiast Box, można użyć innego typu referencyjnego Vec, String, HashMap itp.
    let other_number = number; // przeniesienie własności
    // println!("Value: {}", number) // błąd kompilacji - właścicielem pamięci jest wskaźnik other_number
    // w czasie wykonania, w pamięci nadal są dwie referencje do tego samego obszaru pamięci
    // weryfikacja następuje tylko w czasie kompilacji
}
